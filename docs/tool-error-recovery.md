...
| 调用响应期限耗尽 | `TOOL_TIMEOUT`，说明取消已请求、结果可能未知，先查历史/Task 后决定下一步。 |
...
超过期限会请求取消并返回错误，但不能强制终止忽略取消的 Go 函数，也不能回滚已经发生的写入或外部副作用。工作台真实运行计数保持到处理函数实际退出；晚到的完成事件会用独立、有限时长的记录上下文保存，供之后查询。记录系统本身失效时仍不能保证保存成功。
